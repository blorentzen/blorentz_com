...
export const fn5gl: CaseStudy = {
  ...
  approach:
    "This was a big one. Pre-registration, registration, then the competition itself, each with different requirements and different builds.\n\nI coordinated with an external agency that managed the program data to build APIs for the school directory, interactive map, and dynamic leaderboard (which included the actual voting mechanism). That meant defining the data schema, component requirements, and integration patterns, all within AEM's constraints.\n\nSince this was a high-profile program with its own visual identity, we couldn't use standard T‑Mobile page components. We designed and built custom animations and patterns that matched the Friday Night 5G Lights brand. It had to feel completely different from standard T‑Mobile pages.\n\nI was the Technical Lead. Hired and managed a front-end contractor, provided design and dev direction for all the dynamic components, and coordinated with the API agency on data integration. About 60-70% through, the contractor left for a full-time role... so I picked up where he left off and finished building everything myself since we didn't have enough time to seek and onboard a new developer. I also worked with the agency on a plan for sunsetting the API and bringing the data in-house so the experience could continue displaying results after the 2025 season ended.\n\nThe tech stack was HTML, CSS, and Vanilla JavaScript. For build and testing, we used Vite with custom export patterns to generate code compatible with AEM.",
  result:
